...
export class Simulation {
  ...
  step(dt) {
    ...
    // ---- AUTOCAMPO (carga espacial): aceleración coulombiana par a par O(n^2).
    // Repulsión mutua de electrones: a_i = K * Σ_{j≠i} (r_i-r_j)/(|r_ij|^2+s^2)^{3/2}.
    // coulombK=0 => idéntico al modelo del repo (partícula independiente).
    const K = cfg.coulombK ?? 0;
    if (!this._ax || this._ax.length < this.cap) {
      this._ax = new Float64Array(this.cap);
      this._ay = new Float64Array(this.cap);
      this._az = new Float64Array(this.cap);
    }
    const ax = this._ax, ay = this._ay, az = this._az;
    if (K > 0) {
      const s2 = (cfg.coulombSoft ?? 0.03) ** 2;
      for (let a = 0; a < this.n; a++) { ax[a] = 0; ay[a] = 0; az[a] = 0; }
      for (let a = 0; a < this.n; a++) {
        const a3 = a * 3, axp = this.pos[a3], ayp = this.pos[a3 + 1], azp = this.pos[a3 + 2];
        for (let b = a + 1; b < this.n; b++) {
          const b3 = b * 3;
          const dx = axp - this.pos[b3], dy = ayp - this.pos[b3 + 1], dz = azp - this.pos[b3 + 2];
          const inv = K / Math.pow(dx * dx + dy * dy + dz * dz + s2, 1.5);
          const fx = dx * inv, fy = dy * inv, fz = dz * inv;
          ax[a] += fx; ay[a] += fy; az[a] += fz;          // Newton: i siente +, j siente -
          ax[b] -= fx; ay[b] -= fy; az[b] -= fz;
        }
      }
    }
    ...
    while (i < this.n) {
      ...
      // Empuje de Boris con E y B. Primer medio impulso eléctrico (autocampo):
      let vx = this.vel[ix], vy = this.vel[ix + 1], vz = this.vel[ix + 2];
      if (K > 0) { vx += ax[i] * 0.5 * dt; vy += ay[i] * 0.5 * dt; vz += az[i] * 0.5 * dt; }
      ...
      vy = vy + (vpz * sx - vpx * sz);
      vz = vz + (vpx * sy - vpy * sx);
      // Segundo medio impulso eléctrico (autocampo).
      if (K > 0) { vx += ax[i] * 0.5 * dt; vy += ay[i] * 0.5 * dt; vz += az[i] * 0.5 * dt; }
      ...
      if (r >= cfg.R) {
        ...
        if (i !== last) {
          ...
          ax[i] = ax[last]; ay[i] = ay[last]; az[i] = az[last]; // mover el autocampo precalculado
        }
        this.n--; this.lost++;
